...
namespace Profiler {
	Coord SplineControl::spline_coords(Spline s, double percentage) {
		percentage = MathUtil::max(MathUtil::min(percentage, 1), 0);

		double x = percentage * s.knot_distance;
		double y = (s.a*x + s.b) * (x*x*x*x) + (s.c*x + s.d) * (x*x) + s.e*x;    // Heh, sex <- jaci btw
	...
	double SplineControl::spline_deriv(Spline s, double percentage) {
		double x = percentage * s.knot_distance;
		return (5*s.a*x + 4*s.b) * (x*x*x) + (3*s.c*x + 2*s.d) * x + s.e;
	}

	double SplineControl::spline_deriv_2(double a, double b, double c, double d, double e, double k, double p) {
		double x = p * k;
		return (5*a*x + 4*b) * (x*x*x) + (3*c*x + 2*d) * x + e;
	}

	double SplineControl::spline_angle(Spline s, double percentage) {
		return MathUtil::boundRadians(atan(spline_deriv(s, percentage)) + s.angle_offset);
	}

	double SplineControl::spline_distance(Spline *s, int sample_count) {
	...
	double SplineControl::spline_progress_for_distance(Spline s, double distance, int sample_count) {
		double sample_count_d = (double) sample_count;
